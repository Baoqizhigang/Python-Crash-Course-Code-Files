...
# 在获取用户输入后，Python将其赋值给一个变量，以方便你使用。
...
#7.1.1 编写清晰的程序
# 每当使用函数Input（）的时候，都应当指定清晰易懂的提示，准确地指出希望用户
# 提供什么样的信息 - 指出用户应该如何输入何种信息的任何提示都行。 
# 通过在提示末尾（这里是冒号后面）包含一个空格，可将提示与用户输入分开，
# 让用户清晰地知道其输入始于何处。 

name = input("Please enter you name: ")
print(f"\nHello, {name}!")

# 有时候提示有可能超出一行。 例如，你可能需要指出获取特定输入的原因。 
# 在这种情况下，可将提示赋值给一个变量，再将该变量传递给函数input().
# 这样即使超过一行，input()语句也会非常清晰。
...
year = input("What year it is?")
print(year)
print(type(year))
print(year < 4)
print(year == '3')

year = int(year) #将字符串转化为数值
print(year < 4)


# 假设进入医院的门诊部需要你持有48小时内有效核酸报告
tested_time = input("距离上一次核酸检测多少小时了？")
tested_time = float(tested_time) #将字符串转化为数值(浮点数)

if tested_time <= 48: # 这一行是测试的条件，检查你的HS报告时间是否在48小时内
...
while polling_active: #在首次执行循环时为逻辑真，于是执行下一句代码
    ...
    #将回答存储在字典中
    responses[name] = response #name和response成为键值对，进入response这个字典
...
#  退出的时候，不打印出quit。且仅在消息不是退出值时，才打印该消息。
prompt = "\nTell me something, and I will repeat it back to you:"
prompt +="\nEnter 'quit' to end the program." 
message = " " 
while message != 'quit':
