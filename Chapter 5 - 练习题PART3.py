...
tmp_shop = shop_b # 以这个店铺作为对象来遍历检查，这里需要手动换店铺
...
# 然后通过 random 静态对象调用该方法。
#year = random.random()
year = random.uniform(1970,2025) # 产生1970到2025之间的随机浮点数
year = round(year,0) #四舍五入小数点后保留0位小数
# 括号中的0表示小数点后不保留任何数，但此时的结果仍然是浮点数，类似于1997.0
print(f'你是在{int(year)}年出生的。') #int()可以将浮点数转化为整数
...
# input函数是python的内置函数，它专门用来接收标准输入数据，
# 当用户在终端输入回车后表示输入结束，函数返回值是字符串类型。
year = input("请输入1960~2050之间的四位数字：") # 先运行这条代码，然后再输入四个数字
print(year)
print(type(year)) # 这里变量的类型是string
print(f'你是在{year}年出生的。') 
year = int(year) # 这里把变量的类型转化为整数

if year >= 1960 and year <= 2050:
    zodiac = year % 12
    if zodiac == 0:
        print('你的生肖是：猴')
    if zodiac == 1:
...
if year < 1960 or year > 2050:
    print('请输入在1960~2050之间的四位数字，否则无法得出生肖结果。')

# ------------------------------------ｏｐｔｉｏｎ　4　－－－－－－－－－－－－－－

years = range(1906,2050) # 产生从1906-2050（不含2050）的数字序列

if years:
    for year in years:
        zodiac = year % 12
        if zodiac == 0:
            print(f'{year}年的生肖是：猴')
        if zodiac == 1:
            print(f'{year}年的生肖是：鸡')
        if zodiac == 2:
            print(f'{year}年的生肖是：狗')
        if zodiac == 3:
            print(f'{year}年的生肖是：猪')
        if zodiac == 4:
            print(f'{year}年的生肖是：鼠')
        if zodiac == 5:
            print(f'{year}年的生肖是：牛')
        if zodiac == 6:
            print(f'{year}年的生肖是：虎')
        if zodiac == 7:
            print(f'{year}年的生肖是：兔')
        if zodiac == 8:
            print(f'{year}年的生肖是：龙')
        if zodiac == 9:
            print(f'{year}年的生肖是：蛇')
        if zodiac == 10:
            print(f'{year}年的生肖是：马')
        if zodiac == 11:
            print(f'{year}年的生肖是：羊')
...
第一个数字 = input("请输入一个1~9之间的整数：") # 中文字也是可以成为变量名的
第二个数字 = input("请输入一个1~9之间的整数：") # 中文字也是可以成为变量名的
第三个数字 = input("请输入一个1~9之间的整数：") # 中文字也是可以成为变量名的

第一个数字 = int(第一个数字) # 格式转换
第二个数字 = int(第二个数字)
第三个数字 = int(第三个数字)

digit = 第一个数字*第二个数字*第三个数字
print(f'第一个数字，第二个数字和第三个数字相乘的结果是：{digit}')

digit = str(digit) #把相乘的结果这个整数转换为字符串
print(f'{digit}这个数字现在已经被转换成了{type(digit)}')
...
if len(digit) == 3:
    ...
    numbers.append(digit[2])
    numbers.sort()
    print(f'按升序排列：{numbers[:]}')
    print('这个3位数各个位置上的数字按大小排列是:',numbers[2],numbers[1],numbers[0])
elif len(digit) == 2:
    numbers.append(digit[0])
    numbers.append(digit[1])
    numbers.sort()
    print(f'按升序排列：{numbers[:]}')
    print('这个2位数各个位置上的数字按大小排列是:',numbers[1],numbers[0])
else:
    print(digit)
    print('这个数是:', digit)
...
第一个数字 = input("请输入一个5~9之间的整数：") # 中文字也是可以成为变量名的
第二个数字 = input("请输入一个5~9之间的整数：") # 中文字也是可以成为变量名的
第三个数字 = input("请输入一个5~9之间的整数：") # 中文字也是可以成为变量名的
...
if digit[0] != None: # None在python中是一个特殊的对象，它表示空值，
    numbers.append(digit[0])
if digit[1] != None: # None不支持任何运算，也没有内建方法，除了表示空以外，什么都做不了。
    numbers.append(digit[1])
if digit[2] != None: # 如果一个函数，没有显式return任何数据，则默认返回None。
    # 在判断语句中，None等价于False
    numbers.append(digit[2])
    
numbers.sort() #将数字按升序排列
print(f'按升序排列：{numbers[:]}')
print(f'按降序排列：{sorted(numbers[:], reverse = True)}')
num = sorted(numbers[:], reverse = True)
print('这个3位数各个位置上的数字按大小排列是:',num[0],num[1],num[2])
...
坐标_x = random.uniform(-2,2) # 产生  -2 到 2 之间的随机浮点数，区间可以不是整数
坐标_y = random.uniform(-2,2) # 产生  -2 到 2 之间的随机浮点数，区间可以不是整数

print('这个点的x坐标是：', 坐标_x)
print('这个点的y坐标是：', 坐标_y)

if 坐标_x >=-2 and 坐标_x <= 2 and 坐标_y >=-2 and 坐标_y <= 2:
    print('该点在图形内。')
else:
    print('该点不在图形内。')
...
坐标_x = input('请输入该点的x坐标:')
坐标_y = input('请输入该点的y坐标:')
...
bmi = 体重 / (身高*身高)
bmi = round(bmi,1) #四舍五入小数点后保留1位小数

print (f'这个人身体的BMI指数是：{bmi}')

if bmi < 18.5:
    print ('你的体重过轻')
elif 18.5 <= bmi <24.9: # 等同于: if 18.5 <= bmi and  bmi <24.9
    print('你的体重正常')
elif 24.9 <= bmi < 29.9:
    print ('你的体重过重')
else:
    print('你已属于肥胖体质，请注意调节饮食，加强锻炼。')

print('感谢您使用电子称，欢迎下次再来！')
